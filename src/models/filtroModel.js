...
function insertFiltro(fkEmpresa, nome, data_chegada, fk_continente, fk_pais, fk_via){
    var instrusaoSql = `INSERT INTO TB_FILTRO_DASHBOARD VALUES(default,?,?,?,?,?,?);`
    return database.executar(instrusaoSql, [fkEmpresa, nome, data_chegada,fk_continente,fk_pais,fk_via])
}
...
function updateFiltro(idFiltro, nome, data_chegada, fk_continente, fk_pais, fk_via){
    var instrucaoSql = `UPDATE TB_FILTRO_DASHBOARD
                        SET NOME = ?, DATA_CHEGADA = ?, FK_CONTINENTE = ?, FK_PAIS = ?, FK_VIA = ?
                        WHERE ID_FILTRO = ?`
    return database.executar(instrucaoSql, [nome, data_chegada, fk_continente, fk_pais, fk_via, idFiltro])                
}
